...
1. **单任务计划全文**：把该任务的完整文本提取到独立简报文件（工作区 `.codex/sdd/<变更名>/task-N-brief.md`，工作区约定见 subagent-driven 技能），派发提示指向它——任务文本是需求的唯一来源，精确取值逐字使用；绝不让子代理通读整个计划书
2. **受影响模块 memory 摘要**：读 `.codex/ai-kb/memory/<模块>.md`，把与该任务相关的坑逐条摘进派发提示——前人踩过的坑不重踩
...
- 坑定位并解决后，立即按 `.codex/ai-kb/README.md` 格式追加 `.codex/ai-kb/memory/<模块>.md`，标注来源变更——即时记，不攒到归档
