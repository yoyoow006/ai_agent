...
- **状态在文件里**：`proposal.md` 头部 `状态:` 字段 + `tasks.md` 勾选率。新会话读这两个文件即无损续接，无需额外状态管理。状态全集（8 态，与第 4 节门禁一一对应）：`草稿 → 待确认规范 → 设计中 → 待确认计划 → 构建中 → 待验证 → 待归档 → 已归档`。
...
```
需求 → [Open] → 草稿 → 待确认规范 ──用户确认四件套──→ 设计中 → 待确认计划 ──用户确认计划──→ 构建中
                                                                                          │
    已归档 ←──合并+提交──── 待归档 ←──审查通过──── 待验证 ←──全勾+全绿──────────────────────┘
```
...
执行路径默认走**子代理派发**（主会话保持编排者视角，上下文不被实现细节淹没）；仅当任务极小（单文件微改）或用户明确要求时，主会话按 `executing-plans` 纪律直接执行。
...
4. 变更目录移入 `openspec/archive/<变更名>/`；计划书从 `openspec/plan/` **移入**（非复制）归档目录，`openspec/plan/` 只保留未完成变更的计划
