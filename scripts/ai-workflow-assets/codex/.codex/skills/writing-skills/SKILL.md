...
本仓库的技能存放在 `.codex/skills/<技能名>/SKILL.md`（仓库级技能是本仓库的唯一形态；源技能关于其他运行时技能目录的指引未随本仓库迁移）。
...
**验证：**
```bash
wc -w .codex/skills/<技能名>/SKILL.md
# getting-started 工作流：各 <150 词
# 其他高频载入：总共 <200 词
```
...
- ❌ 坏：`见 .codex/skills/tdd/SKILL.md`（看不出是否必需）
- ❌ 坏：`@.codex/skills/tdd/SKILL.md`（强制加载，烧上下文）
...
### 带可复用工具的技能
```
.codex/skills/condition-based-waiting/
  SKILL.md    # 概述 + 模式
