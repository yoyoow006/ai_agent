...
Open 在进入任一模式前先建立需求共识：代码、测试、OpenSpec、共享知识层和用户明示输入能回答的事实先自行核对；只把会改变目标、范围、风险、术语、约束、验收、验证或授权的问题交给用户。模糊需求按当前已解锁问题分轮编号提问，每题提供证据、选项、具体或条件式推荐与影响；确认后的理解落入 OpenSpec 四件套，不创建平行需求状态。
