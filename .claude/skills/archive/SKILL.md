...
- **memory 归整**：本变更构建与审查期即时记下的坑，去重、按模块归位（`.claude/ai-kb/memory/<模块>.md`，条目格式见 `.claude/ai-kb/README.md`）
- **kb 同步**：本变更改变了模块功能或架构 → 更新 `.claude/ai-kb/kb/<模块>.md`；open 阶段提示过的过时内容在此一并修正
- **rules 更新**：新增模块、别称或关键词 → 更新 `.claude/ai-kb/rules/index.md` 路由表
...
分支的整合归用户决定，不归你——先呈选项、等答复，绝不代用户合并。（收尾纪律源自 finishing-a-development-branch 技能，该技能未随本仓库迁移，纪律内联如下。）
...
3. **选合并**：先确认基分支（通常 main；拿不准就问——合错基分支代价高昂），执行顺序：
   ...
   2. `git checkout main` → `git merge --no-ff feature/<变更名>`
   3. 在合并结果上**再跑一次测试**
...
6. **选 PR**：先做第 6 步提交，`git push -u origin feature/<变更名>` 后按平台工具建 PR；worktree 保留
...
```bash
git add openspec/ .claude/ai-kb/ && git commit -m "chore(archive): <变更名>"
```
