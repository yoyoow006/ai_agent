...
1. **迁移**:旧 `.claude|.codex/ai-kb/memory/{installer.md,workflow-system.md}` 中 7 条 2026-08-16 踩坑条目(2 条 add-workflow-installer、5 条 init-workflow-system)逐字迁入 `.ai/memory/workflow.md`。已验证共享层对这 7 条零命中,不迁移直接删除会丢失真实知识。
...
- **知识层**:共享 memory 净增 7 条历史踩坑 + 1 条审计踩坑;恢复与 origin 迁移后基线(9614f9e)一致的目录形态与门禁绿。
