...
1. **逐字迁移 7 条 memory 条目,不做改写**。旧条目已符合 `memory/README.md` 固定格式(日期·来源变更 + 坑/解);只改措辞会引入无证据的二次失真。条目按日期置于 `workflow.md` 顶部(2026-08-16 早于现存最早条目 2026-08-17)。其中 init-workflow-system 第 5 条("verify 应把状态回写列为收尾步")的建议已在现行 verify 技能落地,作为历史记录原样保留——memory 是追加式历史,不回溯修订。
...
- **知识丢失风险**:已逐条 grep 验证 7 条 memory 在共享层零命中;kb/rules 独有内容确认为废弃描述。残余风险:极低。
