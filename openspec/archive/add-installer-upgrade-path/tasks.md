...
- [x] 1. 台账数据层：独立台账文件 `.ai/installer-ledger.json` 读写、sha256 计算、legacy 识别与 fail-closed 校验（红：无实现时测试失败）
- [x] 2. 升级计划：build_upgrade_plan 四值判定＋REMOVED/KEPT 生成＋入口文件按台账规则判定；SKIPPED 不产生写动作
