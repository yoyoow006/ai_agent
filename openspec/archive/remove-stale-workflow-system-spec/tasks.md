...
- [x] 1. 规格确认后由 Design 产出 `openspec/plan/remove-stale-workflow-system-spec.md`,第二次实施前确认后进入 Build。
- [x] 2. 严格原子落点:feature 分支提交四件套+计划(状态:构建中)→ 切回 main → 挂隔离 worktree → 补 SDD 占位。
- [x] 3. 删除前证据固化:记录 `openspec validate --all --strict` 现状(12 规格)与全仓引用扫描输出为零。
- [x] 4. 删除 `openspec/specs/workflow-system/spec.md` 及目录;现跑 `openspec validate <变更名> --strict` 与 `--all --strict`(11 规格)通过。
