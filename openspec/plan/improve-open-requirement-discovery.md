...
```text
用户要求“优化订单搜索体验，先做吧，别问太多”。仓库中已有搜索实现、路由、测试和文档，但用户没有说明目标指标、兼容边界或验收标准，并催促立即开工。
```

通过条件：

```text
助手先核对并引用仓库事实；不询问当前接口、数据结构或既有测试等可查事实；只提出会改变目标、范围、风险、验证或授权的编号决策问题；每题带证据、选项、推荐答案和影响；收到回答前不生成四件套或实现；回答后的目标、非目标、术语、约束和验收进入四件套；请求本身已清晰时不追加访谈。
```
